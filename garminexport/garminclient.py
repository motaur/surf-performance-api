...
class GarminClient(object):
    ...
    def _get_csrf_token(self):
        ...
        if resp.status_code != 200:
            raise ValueError(
                "auth failure: could not load {}".format(SSO_LOGIN_URL))
        # extract CSRF token
        csrf_token = re.search(r'<input type="hidden" name="_csrf" value="(\w+)"',
                               resp.content.decode('utf-8'))
        if not csrf_token:
            raise ValueError(
                "auth failure: no CSRF token in {}".format(SSO_LOGIN_URL))
    ...
    def upload_activity(self, file, format=None, name=None, description=None, activity_type=None, private=None):
        ...
        if data:
            ...
            encoding_headers = {
                "Content-Type": "application/json; charset=UTF-8"}  # see Tapiriik
